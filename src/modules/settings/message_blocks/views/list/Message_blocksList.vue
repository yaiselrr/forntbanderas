...
import BlocksForm from "~m/settings/message_blocks/views/form/Message_blocksForm.vue"
import RAvatar from "~co/components/avatar/src/RAvatar.vue"
...
const search = ref()
const data = ref([])
const message_blocksTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Message_blocks.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchMessage_blocksList} = Message_blocks.useGetMany({relations: "all"})
...
} = Message_blocks.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchMessage_blocksList()
    message_blocksTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, message_blocksTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("Message_blocksList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("Message_blocksList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Message_blocks.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar bloques de mensajes':'Crear bloques de mensajes' }}</h1>
      </div>
    </template>
    <blocks-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchMessage_blocksList"  :show-loading="true">
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Bloques de mensajes</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Bloques de mensajes</h1>
    </div>
  </div>
    ...
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton v-if="findString('coupons.crud').length > 0" shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="message_blocksTbl?.selectedRowKeys.length===0"
      ...
      <RTable
          ref="message_blocksTbl"
          :columns="columns"
          :data="filter"
          :loading_icon="loading"
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchMessage_blocksList()}"
          :id_element="'id'"
          :expanded="true"
          class="mt-5"
      >
        ...
        <template v-slot:expandedRowRender="{ record }">
          <div class="ml-30" v-html="record.message_text"></div>
        </template>
        <template v-slot:block_image="{_,record}">
          <RAvatar :img="record.block_image" :size="33"
                   :circle="true"/>
        </template>
