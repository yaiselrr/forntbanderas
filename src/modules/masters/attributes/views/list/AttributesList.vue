...
import AttributesForm from "~m/masters/attributes/views/form/AttributesForm.vue"
...
const search = ref()
const data = ref([])
const attributesTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Attributes.visible_columns
...
const {loading, result,onError, onResult, refetch:refetchAttributesList} = Attributes.useGetMany({relations: "all"})
...
} = Attributes.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchAttributesList()
    attributesTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, attributesTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("AttributesList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("AttributesList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Attributes.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar atributo':'Crear atributo' }}</h1>
      </div>
    </template>
    <attributes-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchAttributesList"  :show-loading="true">
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Nomencladores</a-breadcrumb-item>
      <a-breadcrumb-item>Atributos</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Atributos</h1>
    </div>
  </div>
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton v-if="findString('coupons.crud').length > 0" shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="attributesTbl?.selectedRowKeys.length===0"
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    attributesTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ...
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchAttributesList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
        ...
        <template v-slot:expandedRowRender="{ record }">
          <div class="ml-30">{{ record.strcuture }}</div>
        </template>
