...
import Request_statusesForm from "~m/masters/request_statuses/views/form/Request_statusesForm.vue"
...
const search = ref()
const data = ref([])
const request_statusesTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Request_statuses.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchRequest_statusesList} = Request_statuses.useGetMany({relations: "all"})
...
} = Request_statuses.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchRequest_statusesList()
    request_statusesTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, request_statusesTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("Request_statusesList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("Request_statusesList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Request_statuses.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar estado de solicitud':'Crear estado de solicitud' }}</h1>
      </div>
    </template>
    <request_statuses-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchRequest_statusesList"  :show-loading="true">
      <template v-slot:loading_icon>
        <div class="r-loading-default"></div>
      </template>
    </request_statuses-form>
  </a-modal>
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Masters</a-breadcrumb-item>
      <a-breadcrumb-item>Request_statuses</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Estados de solicitud</h1>
    </div>
  </div>
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton v-if="findString('coupons.crud').length > 0" shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="request_statusesTbl?.selectedRowKeys.length===0"
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    request_statusesTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ...
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchRequest_statusesList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
        ...
        <template v-slot:expandedRowRender="{ record }">
          <div class="ml-30" v-html="record.decription"></div>
        </template>
