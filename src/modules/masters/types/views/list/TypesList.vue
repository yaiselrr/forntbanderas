...
import TypesForm from "~m/masters/types/views/form/TypesForm.vue"

const search = ref()
const data = ref([])
const typesTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Types.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchTypesList} = Types.useGetMany({relations: "all"})
...
} = Types.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, typesTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("TypesList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("TypesList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Types.useDelete(record.id)
  onResult(() => {
  ...
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar Types':'Crear Types' }}</h1>
      </div>
    </template>
    <types-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchTypesList"  :show-loading="true">
  ...
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="typesTbl?.selectedRowKeys.length===0"
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    typesTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ...
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchTypesList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
