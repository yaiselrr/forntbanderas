...
import Product_typeForm from "~m/masters/product_type/views/form/Product_typeForm.vue"
...
const search = ref()
const data = ref([])
const product_typeTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Product_type.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchProduct_typeList} = Product_type.useGetMany()
...
} = Product_type.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchProduct_typeList()
    product_typeTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, product_typeTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("Product_typeList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("Product_typeList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Product_type.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar tipo de productos':'Crear tipo de productos' }}</h1>
      </div>
    </template>
    <product_type-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchProduct_typeList"  :show-loading="true">
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Tipos de productos</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Tipo de productos</h1>
    </div>
  </div>
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton v-if="findString('coupons.crud').length > 0" shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="product_typeTbl?.selectedRowKeys.length===0"
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    product_typeTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ...
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchProduct_typeList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
