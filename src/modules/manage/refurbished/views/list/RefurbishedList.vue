...
import RefurbishedForm from '~m/manage/refurbished/views/form/RefurbishedForm.vue'
...
const search = ref()
const data = ref([])
const refurbishedTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns = Refurbished.visible_columns
...
const {
  ...
  onError,
  onResult,
  refetch: refetchRefurbishedList
} = Refurbished.useGetMany({ relations: 'all' })
...
} = Refurbished.useDeleteById(deleteArray, { defaultEnabled: false })
onResult((result) => {
...
onResultDelete(() => {
  refetchRefurbishedList()
  refurbishedTbl?.value?.clearSelection()
  openNotificationWithIcon(
    'success',
    'Acción Exitosa',
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading: loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, refurbishedTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    }),
    icon: null,
...
const exportCSV = () => {
  exportToCSV('RefurbishedList', columns, filter.value)
}
const exportExcel = () => {
  exportToExcelVinstance('RefurbishedList', columns, filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const { onResult, onError } = Refurbished.useDelete(record.id)
  onResult(() => {
    openNotificationWithIcon(
      'success',
      'Acción Exitosa',
...
<template>
  ...
  >
    ...
    <template #title>
      <div class="flex">
        ...
        ></div>
        <h1>{{ selected ? 'Actualizar devoluciones' : 'Crear devoluciones' }}</h1>
      </div>
    </template>
    <refurbished-form
      :modal="true"
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Devoluciones</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Devoluciones</h1>
    </div>
  </div>
    ...
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton
            v-if="findString('refurbished.crud').length > 0"
            shadow
            ref="btnDelete"
            class="btn-delete"
            :disabled="refurbishedTbl?.selectedRowKeys.length === 0"
          ...
          <div class="w-50% mr-20">
            <span class="float-right align-middle"
              >Elementos seleccionados {{ refurbishedTbl?.selectedRowKeys.length }}/{{
                filter.length
              }}</span
            >
          </div>
      ...
      >
        ...
        <template v-slot:_action="{ _, record }">
          <div v-if="findString('refurbished.crud').length > 0" class="flex justify-center align-center">
