...
import Key_wordsForm from '~m/manage/key_words/views/form/Key_wordsForm.vue'
import { ref, computed } from 'vue'

const search = ref()
const data = ref([])
const key_wordsTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns = Key_words.visible_columns
...
const {
  ...
  onError,
  onResult,
  refetch: refetchKey_wordsList
} = Key_words.useGetMany()
...
} = Key_words.useDeleteById(deleteArray, { defaultEnabled: false })
onResult((result) => {
...
onResultDelete(() => {
  refetchKey_wordsList()
  key_wordsTbl?.value?.clearSelection()
  openNotificationWithIcon(
    'success',
    'Acción Exitosa',
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading: loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, key_wordsTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    }),
    icon: null,
...
const exportCSV = () => {
  exportToCSV('Key_wordsList', columns, filter.value)
}
const exportExcel = () => {
  exportToExcelVinstance('Key_wordsList', columns, filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const { onResult, onError } = Key_words.useDelete(record.id)
  onResult(() => {
    openNotificationWithIcon(
      'success',
      'Acción Exitosa',
...
<template>
  ...
  >
    ...
    <template #title>
      <div class="flex">
        ...
        ></div>
        <h1>{{ selected ? 'Actualizar Tipos' : 'Crear Tipos' }}</h1>
      </div>
    </template>
    <key_words-form
      :modal="true"
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Palabras claves</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Palabras claves</h1>
    </div>
  </div>
      ...
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton
            ...
            ref="btnDelete"
            class="btn-delete"
            :disabled="key_wordsTbl?.selectedRowKeys.length === 0"
          ...
          <div class="w-50% mr-20">
            <span class="float-right align-middle"
              >Elementos seleccionados {{ key_wordsTbl?.selectedRowKeys.length }}/{{
                filter.length
              }}</span
            >
          </div>
