...
import Business_sectorsForm from "~m/manage/business_sectors/views/form/Business_sectorsForm.vue"

const search = ref()
const data = ref([])
const business_sectorsTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Business_sectors.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchBusiness_sectorsList} = Business_sectors.useGetMany({relations: "all"})
...
} = Business_sectors.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchBusiness_sectorsList()
    business_sectorsTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, business_sectorsTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("Business_sectorsList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("Business_sectorsList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Business_sectors.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar sectores de negocios':'Crear sectores de negocios' }}</h1>
      </div>
    </template>
    <business_sectors-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchBusiness_sectorsList"  :show-loading="true">
      <template v-slot:loading_icon>
        <div class="r-loading-default"></div>
      </template>
    </business_sectors-form>
  </a-modal>
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Sectores de negocios</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Sectores de negocios</h1>
    </div>
  </div>
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="business_sectorsTbl?.selectedRowKeys.length===0"
      ...
      <RTable
          ref="business_sectorsTbl"
          :columns="columns"
          :data="filter"
          :loading_icon="loading"
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchBusiness_sectorsList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
