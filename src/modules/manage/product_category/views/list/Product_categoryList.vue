...
import Product_categoryForm from "~m/manage/product_category/views/form/Product_categoryForm.vue"

const search = ref()
const data = ref([])
const product_categoryTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Product_category.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchProduct_categoryList} = Product_category.useGetMany({relations: "all"})
...
} = Product_category.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchProduct_categoryList()
    product_categoryTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, product_categoryTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("Product_categoryList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("Product_categoryList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Product_category.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar Product_category':'Crear Product_category' }}</h1>
      </div>
    </template>
    <product_category-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchProduct_categoryList"  :show-loading="true">
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Product_category</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Product_category</h1>
    </div>
  </div>
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="product_categoryTbl?.selectedRowKeys.length===0"
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    product_categoryTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ref="product_categoryTbl"
          :columns="columns"
          :data="filter"
          :loading_icon="loading"
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchProduct_categoryList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
