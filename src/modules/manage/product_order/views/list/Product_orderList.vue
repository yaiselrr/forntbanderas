...
import Product_orderForm from '~m/manage/product_order/views/form/Product_orderForm.vue'

const search = ref()
const data = ref([])
const product_orderTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns = Product_order.visible_columns
...
const {
  ...
  onError,
  onResult,
  refetch: refetchProduct_orderList
} = Product_order.useGetMany({ relations: 'all' })
...
} = Product_order.useDeleteById(deleteArray, { defaultEnabled: false })
onResult((result) => {
...
onResultDelete(() => {
  refetchProduct_orderList()
  product_orderTbl?.value?.clearSelection()
  openNotificationWithIcon(
    'success',
    'Acción Exitosa',
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading: loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, product_orderTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    }),
    icon: null,
...
const exportCSV = () => {
  exportToCSV('Product_orderList', columns, filter.value)
}
const exportExcel = () => {
  exportToExcelVinstance('Product_orderList', columns, filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const { onResult, onError } = Product_order.useDelete(record.id)
  onResult(() => {
    openNotificationWithIcon(
      'success',
      'Acción Exitosa',
...
<template>
  ...
  >
    ...
    <template #title>
      <div class="flex">
        ...
        ></div>
        <h1>{{ selected ? 'Actualizar Product_order' : 'Crear Product_order' }}</h1>
      </div>
    </template>
    <product_order-form
      :modal="true"
      ...
      @reload="refetchProduct_orderList"
      :show-loading="true"
    >
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Órdenes Productos</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Órdenes Productos</h1>
    </div>
  </div>
      ...
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton
            ...
            ref="btnDelete"
            class="btn-delete"
            :disabled="product_orderTbl?.selectedRowKeys.length === 0"
          ...
          <RButton class="mr-3" shadow @click="createElmenent">
            ...
            Crear Órdenes Productos
          </RButton>
          <div class="w-50% mr-20">
            <span class="float-right align-middle"
              >Elementos seleccionados {{ product_orderTbl?.selectedRowKeys.length }}/{{
                filter.length
              }}</span
            >
          </div>
