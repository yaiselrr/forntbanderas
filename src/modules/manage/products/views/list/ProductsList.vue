...
import ProductsForm from '~m/manage/products/views/form/ProductsForm.vue'
...
const search = ref()
const data = ref([])
const productsTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns = Products.visible_columns
...
const { loading, result, onError, onResult, refetch: refetchProductsList } = Products.useGetMany()
...
} = Products.useDeleteById(deleteArray, { defaultEnabled: false })
onResult((result) => {
...
onResultDelete(() => {
  refetchProductsList()
  productsTbl?.value?.clearSelection()
  openNotificationWithIcon(
    'success',
    'Acción Exitosa',
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading: loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, productsTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    }),
    icon: null,
...
const exportCSV = () => {
  exportToCSV('ProductsList', columns, filter.value)
}
const exportExcel = () => {
  exportToExcelVinstance('ProductsList', columns, filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const { onResult, onError } = Products.useDelete(record.id)
  onResult(() => {
    openNotificationWithIcon(
      'success',
      'Acción Exitosa',
  ...
  >
    ...
    <template #title>
      <div class="flex">
        ...
        ></div>
        <h1>{{ selected ? 'Actualizar productos' : 'Crear productos' }}</h1>
      </div>
    </template>
    <products-form
      :modal="true"
    ...
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton
            v-if="findString('products.crud').length > 0"
            shadow
            ref="btnDelete"
            class="btn-delete"
            :disabled="productsTbl?.selectedRowKeys.length === 0"
          ...
          <RButton
            ...
            class="mr-3"
            shadow
            @click="router.push({ path: '/manage/products/create' })"
          >
          ...
          <div class="w-50% mr-20">
            <span class="float-right align-middle"
              >Elementos seleccionados {{ productsTbl?.selectedRowKeys.length }}/{{
                filter.length
              }}</span
            >
          </div>
      ...
      >
        ...
        <template v-slot:_action="{ _, record }">
          <div v-if="findString('products.crud').length > 0" class="flex justify-center align-center">
