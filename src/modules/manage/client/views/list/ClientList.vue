...
const columns = Client.visible_columns
...
} = Client.useDeleteById(deleteArray, { defaultEnabled: false })
onResult((result) => {
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading: loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, clientTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    }),
    icon: null,
...
const exportCSV = () => {
  exportToCSV('ClientList', columns, filter.value)
}
const exportExcel = () => {
  exportToExcelVinstance('ClientList', columns, filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const { onResult, onError } = Client.useDelete(record.id)
  onResult(() => {
    openNotificationWithIcon(
      'success',
      'Acción Exitosa',
  ...
  >
    ...
    <template #title>
      <div class="flex">
        ...
        ></div>
        <h1>{{ selected ? 'Actualizar Client' : 'Crear Client' }}</h1>
      </div>
    </template>
    <client-form
      :modal="true"
    ...
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton
            v-if="findString('manage.crud').length > 0"
            shadow
            ref="btnDelete"
            class="btn-delete"
      ...
      >
        ...
        <template v-slot:_action="{ _, record }">
          <div v-if="findString('manage.crud').length > 0" class="flex justify-center align-center">
