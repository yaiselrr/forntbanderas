...
import InvoicesForm from '~m/manage/invoices/views/form/InvoicesForm.vue'
...
const search = ref()
const data = ref([])
const invoicesTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns = Invoices.visible_columns
...
} = Invoices.useGetMany({ relations: 'all' })
...
} = Invoices.useDeleteById(deleteArray, { defaultEnabled: false })
onResult((result) => {
...
onResultDelete(() => {
  refetchInvoicesList()
  invoicesTbl?.value?.clearSelection()
  openNotificationWithIcon(
    'success',
    'Acción Exitosa',
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading: loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, invoicesTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    }),
    icon: null,
...
const exportCSV = () => {
  exportToCSV('InvoicesList', columns, filter.value)
}
const exportExcel = () => {
  exportToExcelVinstance('InvoicesList', columns, filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const { onResult, onError } = Invoices.useDelete(record.id)
  onResult(() => {
    openNotificationWithIcon(
      'success',
      'Acción Exitosa',
...
<template>
  ...
  >
    ...
    <template #title>
      <div class="flex">
        ...
        ></div>
        <h1>{{ selected ? 'Actualizar facturas' : 'Crear facturas' }}</h1>
      </div>
    </template>
    <invoices-form
      :modal="true"
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Facturas</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
    ...
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton
            v-if="findString('invoices.crud').length > 0"
            shadow
            ref="btnDelete"
            class="btn-delete"
            :disabled="invoicesTbl?.selectedRowKeys.length === 0"
          ...
          <div class="w-50% mr-20">
            <span class="float-right align-middle"
              >Elementos seleccionados {{ invoicesTbl?.selectedRowKeys.length }}/{{
                filter.length
              }}</span
            >
          </div>
      ...
      >
        ...
        <template v-slot:_action="{ _, record }">
          <div v-if="findString('invoices.crud').length > 0" class="flex justify-center align-center">
