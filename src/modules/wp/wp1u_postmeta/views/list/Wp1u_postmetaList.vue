...
import Wp1u_postmetaForm from "~m/wp/wp1u_postmeta/views/form/Wp1u_postmetaForm.vue"

const search = ref()
const data = ref([])
const wp1u_postmetaTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Wp1u_postmeta.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchWp1u_postmetaList} = Wp1u_postmeta.useGetMany({relations: "all"})
...
} = Wp1u_postmeta.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchWp1u_postmetaList()
    wp1u_postmetaTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, wp1u_postmetaTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("Wp1u_postmetaList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("Wp1u_postmetaList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Wp1u_postmeta.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar Wp1u_postmeta':'Crear Wp1u_postmeta' }}</h1>
      </div>
    </template>
    <wp1u_postmeta-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchWp1u_postmetaList"  :show-loading="true">
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Wp1u_postmeta</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Wp1u_postmeta</h1>
    </div>
  </div>
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="wp1u_postmetaTbl?.selectedRowKeys.length===0"
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    wp1u_postmetaTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ref="wp1u_postmetaTbl"
          :columns="columns"
          :data="filter"
          :loading_icon="loading"
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchWp1u_postmetaList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
