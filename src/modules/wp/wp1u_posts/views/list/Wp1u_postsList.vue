...
import Wp1u_postsForm from "~m/wp/wp1u_posts/views/form/Wp1u_postsForm.vue"

const search = ref()
const data = ref([])
const wp1u_postsTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Wp1u_posts.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchWp1u_postsList} = Wp1u_posts.useGetMany({relations: "all"})
...
} = Wp1u_posts.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchWp1u_postsList()
    wp1u_postsTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, wp1u_postsTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("Wp1u_postsList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("Wp1u_postsList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Wp1u_posts.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar Wp1u_posts':'Crear Wp1u_posts' }}</h1>
      </div>
    </template>
    <wp1u_posts-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchWp1u_postsList"  :show-loading="true">
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Wp1u_posts</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Wp1u_posts</h1>
    </div>
  </div>
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="wp1u_postsTbl?.selectedRowKeys.length===0"
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    wp1u_postsTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ...
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchWp1u_postsList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
