...
import Wp1u_yoast_indexableForm from "~m/wp/wp1u_yoast_indexable/views/form/Wp1u_yoast_indexableForm.vue"

const search = ref()
const data = ref([])
const wp1u_yoast_indexableTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Wp1u_yoast_indexable.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchWp1u_yoast_indexableList} = Wp1u_yoast_indexable.useGetMany({relations: "all"})
...
} = Wp1u_yoast_indexable.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const showModal = (refresh = false) => {
  ...
  if (refresh) {
    Modal.destroyAll()
    refetchWp1u_yoast_indexableList()
  }
}
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchWp1u_yoast_indexableList()
    wp1u_yoast_indexableTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, wp1u_yoast_indexableTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("Wp1u_yoast_indexableList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("Wp1u_yoast_indexableList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Wp1u_yoast_indexable.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar Wp1u_yoast_indexable':'Crear Wp1u_yoast_indexable' }}</h1>
      </div>
    </template>
    <wp1u_yoast_indexable-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchWp1u_yoast_indexableList"  :show-loading="true">
      <template v-slot:loading_icon>
        <div class="r-loading-default"></div>
      </template>
    </wp1u_yoast_indexable-form>
  </a-modal>
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Wp</a-breadcrumb-item>
      <a-breadcrumb-item>Wp1u_yoast_indexable</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Wp1u_yoast_indexable</h1>
    </div>
  </div>
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="wp1u_yoast_indexableTbl?.selectedRowKeys.length===0"
      ...
      <RTable
          ref="wp1u_yoast_indexableTbl"
          :columns="columns"
          :data="filter"
          :loading_icon="loading"
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchWp1u_yoast_indexableList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
