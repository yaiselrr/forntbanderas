...
import Pay_waysForm from "~m/sale/pay_ways/views/form/Pay_waysForm.vue"
...
const search = ref()
const data = ref([])
const pay_waysTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Pay_ways.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchPay_waysList} = Pay_ways.useGetMany({relations: "all"})
...
} = Pay_ways.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchPay_waysList()
    pay_waysTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, pay_waysTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("Pay_waysList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("Pay_waysList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Pay_ways.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar formas de pago':'Crear formas de pago' }}</h1>
      </div>
    </template>
    <pay_ways-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchPay_waysList"  :show-loading="true">
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Sale</a-breadcrumb-item>
      <a-breadcrumb-item>Formas de pagos</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Formas de pagos</h1>
    </div>
  </div>
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton v-if="findString('coupons.crud').length > 0" shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="pay_waysTbl?.selectedRowKeys.length===0"
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    pay_waysTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ...
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchPay_waysList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
