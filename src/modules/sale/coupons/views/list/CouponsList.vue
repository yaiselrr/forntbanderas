...
import CouponsForm from "~m/sale/coupons/views/form/CouponsForm.vue"
...
const search = ref()
const data = ref([])
const couponsTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns=Coupons.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchCouponsList} = Coupons.useGetMany({relations: "all"})
...
} = Coupons.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const closeModal = (refresh=false) => {
  ...
  onResultDelete(() => {
    refetchCouponsList()
    couponsTbl?.value?.clearSelection()
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, couponsTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("CouponsList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("CouponsList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Coupons.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar cupones':'Crear cupones' }}</h1>
      </div>
    </template>
    <coupons-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchCouponsList"  :show-loading="true">
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Ventas</a-breadcrumb-item>
      <a-breadcrumb-item>Cupones</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      <h1 class="mt-5">Cupones</h1>
    </div>
  </div>
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton v-if="findString('coupons.crud').length > 0" shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="couponsTbl?.selectedRowKeys.length===0"
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    couponsTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ...
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchCouponsList()}"
          :id_element="'id'"
          :expanded="true"
          class="mt-5"
      >
        ...
        <template v-slot:expandedRowRender="{ record }">
          <div class="ml-30" v-html="record.conditions"></div>
        </template>
