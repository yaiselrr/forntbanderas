...
import RolesForm from "~m/security/roles/views/form/RolesForm.vue"
import { siteStore } from '~cf/store/site.store'
import RolesFormPermissions from '~m/security/roles/views/form/RolesFormPermissions.vue'
...
const search = ref()
const data = ref([])
const rolesTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns = Roles.visible_columns
const filter = computed!(() => {
  return data.value.filter((e) => filter_object_column(e, search.value, columns))
})

const {loading, result, onError, onResult, refetch: refetchRolesList} = Roles.useGetMany({relations: "all"})
...
} = Roles.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading: loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, rolesTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV = () => {
  exportToCSV("RolesList", columns, filter.value)
}
const exportExcel = () => {
  exportToExcelVinstance("RolesList", columns, filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Roles.useDelete(record.id)
  onResult(() => {
...
<template>
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        <div :class="selected?'i-heroicons:pencil-square':'i-heroicons:plus-20-solid'"
             class="w-1em h-1em p-3.5 mr-2"></div>
        <h1>{{ selected ? "Actualizar roles" : "Crear roles" }}</h1>
      </div>
    </template>
    <roles-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
                @reload="refetchRolesList" :show-loading="true">
  ...
  <div class="r-container-list">
    <div class="relative card w-full float-right">
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton v-if="findString('coupons.crud').length > 0" shadow ref="btnDelete"
                   class="btn-delete"
                   :disabled="rolesTbl?.selectedRowKeys.length===0"
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    rolesTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ...
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchRolesList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
          ...
          <div v-if="findString('coupons.crud').length > 0" class="flex justify-center align-center">
            <div class=" text-xl flex justify-center align-center">
              <a-tooltip>
                ...
                <div class="i-heroicons:pencil-square w-1em h-1em text-blue-600 cursor-pointer"
                     @click="updateElement(record)"></div>
              </a-tooltip>
              <a-popconfirm
