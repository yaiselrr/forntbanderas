...
const columns=Dealers.visible_columns
const filter = computed!(() => {
  return data.value.filter((e)=>filter_object_column(e,search.value,columns))
})

const {loading, result,onError, onResult, refetch:refetchDealersList} = Dealers.useGetMany({relations: "all"})
...
} = Dealers.useDeleteById(deleteArray, {defaultEnabled: false})
onResult((result) => {
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading:loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, dealersTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    },),
    icon: null,
...
const exportCSV=()=>{
  exportToCSV("DealersList",columns,filter.value)
}
const exportExcel=()=>{
  exportToExcelVinstance("DealersList",columns,filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const {onResult, onError} = Dealers.useDelete(record.id)
  onResult(() => {
  ...
  <a-modal v-model:open="openModal" :footer="null"
    ...
    <template #title>
      <div class="flex">
        ...
        <h1>{{ selected?'Actualizar distribuidores':'Crear distribuidores' }}</h1>
      </div>
    </template>
    <dealers-form :modal="true" v-model="selected" :action="selected?'update':'create'" @closeModal="closeModal"
             @reload="refetchDealersList"  :show-loading="true">
  ...
  <div class="r-container-list">
    <div class="relative card w-full float-right">
        ...
        <div class="components-input-demo-presuffix mt-3 flex">
          ...
          <div class="w-50% mr-20">
                <span class="float-right align-middle">Elementos seleccionados {{
                    dealersTbl?.selectedRowKeys.length
                  }}/{{ filter.length }}</span>
          </div>
      ...
      <RTable
          ...
          :has_selection="true"
          :on_select="rec=>{}"
          @change-pagination="(e)=>{refetchDealersList()}"
          :id_element="'id'"
          class="mt-5"
      >
        <template v-slot:loading_icon>
