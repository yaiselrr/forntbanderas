...
import PermissionsForm from '~m/security/permissions/views/form/PermissionsForm.vue'
...
const search = ref()
const data = ref([])
const permissionsTbl = ref<HTMLDivElement | null>(null)
const selected = ref(null)
const columns = Permissions.visible_columns
...
} = Permissions.useGetMany({ relations: 'all' })
...
} = Permissions.useDeleteById(deleteArray, { defaultEnabled: false })
onResult((result) => {
...
onResultDelete(() => {
  refetchPermissionsList()
  permissionsTbl?.value?.clearSelection()
  openNotificationWithIcon(
    'success',
    'Acción Exitosa',
...
const deleteElements = () => {
  Modal.confirm({
    title: createVNode(ConfirmModal, {
      ...
      loading: loadingDelete,
      onOk: () => {
        Object.assign(deleteArray, permissionsTbl?.value?.selectedRowKeys)
        toggleEnabledDelete()
      }
    }),
    icon: null,
...
const exportCSV = () => {
  exportToCSV('PermissionsList', columns, filter.value)
}
const exportExcel = () => {
  exportToExcelVinstance('PermissionsList', columns, filter.value)
}
const confirmDelete = (record) => {
  loading.value = true
  const { onResult, onError } = Permissions.useDelete(record.id)
  onResult(() => {
    openNotificationWithIcon(
      'success',
      'Acción Exitosa',
...
<template>
  ...
  >
    ...
    <template #title>
      <div class="flex">
        ...
        ></div>
        <h1>{{ selected ? 'Actualizar permisos' : 'Crear permisos' }}</h1>
      </div>
    </template>
    <permissions-form
      :modal="true"
  ...
  <div class="r-breadcumbs">
    <a-breadcrumb class="h-full">
      ...
      <a-breadcrumb-item>Permisos</a-breadcrumb-item>
    </a-breadcrumb>
    <div>
      ...
      <div class="">
        <div class="components-input-demo-presuffix mt-3 flex">
          <RButton v-if="findString('coupons.crud').length > 0"
            ...
            ref="btnDelete"
            class="btn-delete"
            :disabled="permissionsTbl?.selectedRowKeys.length === 0"
          ...
          <div class="w-50% mr-20">
            <span class="float-right align-middle"
              >Elementos seleccionados {{ permissionsTbl?.selectedRowKeys.length }}/{{
                filter.length
              }}</span
            >
          </div>
