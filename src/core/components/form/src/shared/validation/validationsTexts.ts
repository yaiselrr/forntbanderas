...
export const PASSWORD_ONE_UPPER = 'La contraseña debe tener al menos una mayúscula'
export const PASSWORD_ONE_LOWER = 'La contraseña debe tener al menos una minúscula'
...
export const getTextValidation = (validator, params) => {
    ...
    if (validator == 'maxLength') {
        ...
        return MIN_LENGTH(params.min)
    } else if (validator == 'email') {
        return EMAIL_VALID
    } else if (validator == 'minValue') {
        return MIN_VALUE(params.min)
    }
    return TextMap[validator] ? TextMap[validator] : 'Ingrese un valor válido para este campo'
}
