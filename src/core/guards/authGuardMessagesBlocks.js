...
const authGuardMessagesBlocks = (to, from, next) => {
  ...
  if (isAuth) {
    if (user) {
      let permissions = user.array_permission.map(item => item.name).filter((element) => {
        return element.includes('messages_blocks.crud')
      })
      let permissionsRead = user.array_permission.map(item => item.name).filter((element) => {
        return element.includes('messages_blocks.read')
      })
...
export default authGuardMessagesBlocks
