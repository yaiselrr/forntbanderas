...
const authGuardAttributes = (to, from, next) => {
  ...
  if (isAuth) {
    if (user) {
      let permissions = user.array_permission.map(item => item.name).filter((element) => {
        return element.includes('attributes.crud')
      })
      let permissionsRead = user.array_permission.map(item => item.name).filter((element) => {
        return element.includes('attributes.read')
      })
...
export default authGuardAttributes
