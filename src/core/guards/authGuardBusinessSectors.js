...
const authGuardBusinessSectors = (to, from, next) => {
  ...
  if (isAuth) {
    if (user) {
      let permissions = user.array_permission.map(item => item.name).filter((element) => {
        return element.includes('business_sectors.crud')
      })
      let permissionsRead = user.array_permission.map(item => item.name).filter((element) => {
        return element.includes('business_sectors.read')
      })
...
export default authGuardBusinessSectors
