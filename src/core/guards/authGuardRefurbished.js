...
const authGuardRefurbished = (to, from, next) => {
  ...
  if (isAuth) {
    if (user) {
      let permissions = user.array_permission.map(item => item.name).filter((element) => {
        return element.includes('refurbished.crud')
      })
      let permissionsRead = user.array_permission.map(item => item.name).filter((element) => {
        return element.includes('refurbished.read')
      })
...
export default authGuardRefurbished
