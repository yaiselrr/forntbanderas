...
const authGuardProductTypes = (to, from, next) => {
  ...
  if (isAuth) {
    if (user) {
      let permissions = user.array_permission.map(item => item.name).filter((element) => {
        return element.includes('product_types.crud')
      })
      let permissionsRead = user.array_permission.map(item => item.name).filter((element) => {
        return element.includes('product_types.read')
      })
...
export default authGuardProductTypes
